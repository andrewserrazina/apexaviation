...
--
-- ===========================================================================
-- REV3 CHANGE NOTE -- every score component scoped to APPLICABLE tasks only
-- ===========================================================================
-- Independent review found that, even after Rev2 fixed the coverage
-- denominator to be "all tasks in the active ACS version" instead of "only
-- tasks Apex has content for," it was still measuring against ALL 61
-- authoritative tasks -- including seaplane-only and multiengine-only tasks
-- that do not apply to Apex's actual ASEL learners at all. Worse, every
-- OTHER score component (knowledge_score, risk_management_score,
-- confidence_score, evidence_volume, weak_tasks) queried `task_evidence`
-- with no task filter beyond `profile_id`, meaning evidence on a
-- non-applicable task, or (in the future) evidence recorded against a
-- retired ACS version or a different certificate track, would silently
-- blend into a PPL ASEL learner's current readiness number.
--
-- Rev3 threads `public.get_applicable_acs_tasks(v_profile_id)` (v112
-- REV3.4 -- the ONE resolution path, not a second slightly-different
-- applicability query written here) through every one of the six score
-- components below. A learner with evidence recorded against a
-- non-applicable task (e.g. an ASES-only task, if it were ever mapped and
-- attempted) will simply never have that evidence counted here -- proven in
-- the regression suite by seeding exactly that scenario and asserting the
-- resulting score is identical to the same learner with no such evidence
-- at all.
--
-- Old readiness_snapshots rows are NOT rewritten -- they remain historical
-- exactly as computed at the time, per REV3.6's explicit instruction not to
-- rewrite history.
--
...
-- task_evidence/acs_tasks/acs_versions/acs_task_applicability (v112/v113),
-- never modifies them.
...
begin
  ...
  -- REV3: total/covered/content-less are all counted over APPLICABLE
  -- tasks only (get_applicable_acs_tasks resolves the learner's own
  -- aircraft_class internally -- no separate applicability query here).
  select count(*) into v_total_tasks
  from public.get_applicable_acs_tasks(v_profile_id);

  select count(distinct e.acs_task_id) into v_covered_tasks
  from public.task_evidence e
  join public.get_applicable_acs_tasks(v_profile_id) t on t.id = e.acs_task_id
  ...
  -- REV2.14 (REV3: denominator narrowed to applicable tasks): a task Apex
  -- has zero content mapped to is not the same thing as a task the learner
  -- hasn't studied -- flag the limitation explicitly. A seaplane-only task
  -- Apex has no content for is NOT flagged here for an ASEL learner (it is
  -- not in their applicable set at all); a genuinely content-less
  -- APPLICABLE task is.
  ...
  -- REV3: knowledge/risk/confidence/evidence_volume all restricted to
  -- task_evidence rows whose acs_task_id is in the learner's own
  -- applicable set -- evidence on a non-applicable task (or, in the
  -- future, a different ACS version/certificate track) never blends in.
  select round(100.0 * avg(e.evidence_score), 2), coalesce(sum(e.attempt_count), 0)
  ...
  from public.task_evidence e
  join public.get_applicable_acs_tasks(v_profile_id) t on t.id = e.acs_task_id
  where e.profile_id = v_profile_id and e.confidence_alignment is not null;
  ...
  -- REV3: weak_tasks is drawn only from the learner's applicable tasks --
  -- a non-applicable task's evidence (should any ever exist) must never
  -- surface as a "weak area" the mobile app tells an ASEL learner to drill.
